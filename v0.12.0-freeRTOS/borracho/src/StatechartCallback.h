...
#define LEDC_CHANNEL_PWM_HEATER 0 // Usar o canal 0 do LEDC (ESP32 tem vários canais PWM)
// LEDC_TIMER_BIT_PWM_HEATER e LEDC_BASE_FREQ_PWM_HEATER serão lidos do Yakindu, mas podemos ter defaults.
...
public:
  ...
  void initializeProcess() override
  {
    ...
    // TODO: Aqui faria qualquer inicialização de atuadores/sensores antes do ciclo de controle
  ...
  void startNextRecipeStep(sc_integer recipeIndex) override
  {
    ...
    if (this->currentRecipeIdx >= 0 && this->currentRecipeIdx < NUM_RECIPES)
    {
      ...
      {
        // Este bloco não deveria ser alcançado se as guardas do Yakindu estiverem corretas -- Debug
  ...
  // --- NOVOS MÉTODOS DE CALLBACK PARA CONTROLE DO AQUECEDOR PWM ---
  /**
   * @brief Configura o pino GPIO especificado para operar como saída PWM para o aquecedor.
   * Chamado pelo Yakindu (ex: no INIT_SYSTEM).
   */
  void setupHeaterPWM() override
  {
    ...
    if (myStatechart != nullptr)
    {
      int heaterPin = myStatechart->getHeater_pwm_pin();       // Pego do Yakindu
      int pwmFreq = myStatechart->getPwm_frequency();          // Pego do Yakindu
      int pwmResBits = myStatechart->getPwm_resolution_bits(); // Pego do Yakindu
   ...
   * Esta função é chamada pela operação 'heat' do Yakindu e pela 'controlTask'.
  ...
  // --- MÉTODOS DE CUSTOMIZAÇÃO (NÃO PRIORIZADO AGORA) ---
  void showCustomSetup_GetNumSteps() override {}
  sc_boolean isValidNumSteps(sc_integer) override {}
